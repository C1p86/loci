...
- **D-06:** **Explicit plugin registration order, no autoload.** Server is small in Phase 7; `@fastify/autoload` adds magic without payoff. Order: `@fastify/env` → DB plugin (`db-plugin.ts` decorates `fastify.db`) → `@fastify/helmet` (security headers — must register before any response is sent) → `@fastify/cookie` → `@fastify/csrf-protection` → `@fastify/rate-limit` → auth plugin (decorates request) → error-handler plugin (centralized `setErrorHandler` mapping `XciServerError` to HTTP — must register before routes so route throws are caught) → routes (registered with `prefix: '/api'`). Phase 8+ adds `@fastify/websocket` after auth plugin. (Original draft of this decision listed the 7 core plugins; helmet + error-handler were added during planning per security best practice — keep them.)
