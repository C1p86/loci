...
- [x] 13-03-PLAN.md — Agents list + empty-state with reg-token + Tasks list + Monaco YAML editor (lazy) + inline validation + trigger form + run detail shell
- [x] 13-04-PLAN.md — LogViewer with autoscroll pause/resume + WS subscription + reconnect with sinceSeq + timestamp toggle + download + History with filters
- [x] 13-05-PLAN.md — Settings Org (members/invites/usage/leave) + Plugin Settings (webhook tokens) + DLQ list + retry
- [x] 13-06-PLAN.md — CI web gates + Playwright Linux smoke + READMEs + STATE/ROADMAP/REQUIREMENTS traceability + 13-CLOSEOUT-SUMMARY + human-verify checkpoint
**UI hint**: yes
...
| 14. Docker & Publishing | 1/4 | In Progress|  |
